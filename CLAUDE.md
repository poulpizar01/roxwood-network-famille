...
- **Restent fixes dans le code, volontairement pas de `/config` dédié** — valeurs jugées suffisamment stables une fois le bot déployé pour une organisation donnée :
  ...
  - Plafonds indicatifs de munitions et amende de fourrière (`src/modules/armurerie.ts`, `src/modules/garages.ts`) — purement informatifs, aucune facturation automatique nulle part dans le bot. Le stock réel de munitions affiché dans l'embed armurerie vient d'un item `/config item add` normal, associé via son `groupe` à la constante fixe `MUNITIONS_STOCK_GROUP` (`'Munitions de pistolet'`, câblée dans `armurerie.ts`) — volontairement un libellé de groupe choisi par l'admin, pas un nom d'item exact (contrairement au piège n°1), pour ne pas dépendre de l'orthographe précise des logs FiveM.
