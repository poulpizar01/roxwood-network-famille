...
4. Le serveur Express écoute sur toutes les interfaces (`0.0.0.0:<API_PORT>`, pas seulement en local) — en prod, mettre un reverse proxy HTTPS devant (ex. Caddy : `bot.exemple.fr { reverse_proxy 127.0.0.1:3001 }`, certificat automatique) et **restreindre `<API_PORT>` par pare-feu** pour qu'il ne soit joignable que depuis la machine elle-même (n'ouvrir que le port 443 au pare-feu — même logique que le port `5432` de PostgreSQL en Docker, voir `docker-compose.yml`) ; `API_BASE_URL` doit alors pointer vers ce domaine public, pas vers `localhost`. Vérifier : `curl https://<API_BASE_URL>/health` doit répondre `{"ok":true}`.
